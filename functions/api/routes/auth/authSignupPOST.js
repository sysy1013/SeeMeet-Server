  ...
  try {
    ...
    if(userFirebase.err){
        if(userFirebase.error.code === 'auth/email-already-exists'){
          ...
          if(!checkuser){
            return res.status(statusCode.CONFLICT).json(util.fail(statusCode.CONFLICT, '해당 이메일을 가진 유저가 이미 있습니다.'));
          }else{
            ...
            return res.status(statusCode.NOT_ACCESS).json(util.fail(statusCode.NOT_ACCESS, ' 비밀번호 형식이 잘못되었습니다. 패스워드는 최소 6자리의 문자열이어야합니다.')); 
        }else if(userFirebase.error.code === 'auth/invalid-email'){
            return res.status(statusCode.FORBIDDEN).json(util.fail(statusCode.FORBIDDEN, '이메일 형식이 올바르지 않습니다.'))
        }else {
