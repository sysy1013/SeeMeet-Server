  ...
  try {
    ...
    if(userFirebase.err){
        if(userFirebase.error.code === 'auth/user-not-found'){
            ...
            return res.status(statusCode.NOT_FOUND).json(util.fail(statusCode.NOT_ACCESS,responseMessage.INVALID_EMAIL));
        }else if(userFirebase.error.code === 'auth/wrong-password'){
            return res.status(statusCode.NOT_FOUND).json(util.fail(statusCode.FORBIDDEN,responseMessage.MISS_MATCH_PW));
        }else{
